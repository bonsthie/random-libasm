...
static x86_simd_support_t x86_detect_cpu_features() {
  ...
  if (__get_cpuid_count__(7, 0, &eax, &ebx, &ecx, &edx)) {
    ...
    x86_simd_support.erms = ebx & Bit_ERMS ? 1 : 0;
  }
...
void x86_print_features() {
  ...
  if (x86_simd_support.mmx)
    printf("-mmmx ");
  if (x86_simd_support.sse)
    printf("-msse ");
  if (x86_simd_support.sse2)
    printf("-msse2 ");
  if (x86_simd_support.sse3)
    printf("-msse3 ");
  if (x86_simd_support.ssse3)
    printf("-mssse3 ");
  if (x86_simd_support.sse41)
    printf("-msse4.1 ");
  if (x86_simd_support.sse42)
    printf("-msse4.2 ");
  if (x86_simd_support.avx)
    printf("-mavx ");
  if (x86_simd_support.avx2)
    printf("-mavx2 ");
  if (x86_simd_support.avx512f)
    printf("-mavx512f ");
  if (x86_simd_support.avx512dq)
    printf("-mavx512dq ");
  if (x86_simd_support.avx512ifma)
    printf("-mavx512ifma ");
  if (x86_simd_support.avx512pf)
    printf("-mavx512pf ");
  if (x86_simd_support.avx512er)
    printf("-mavx512er ");
  if (x86_simd_support.avx512cd)
    printf("-mavx512cd ");
  if (x86_simd_support.avx512bw)
    printf("-mavx512bw ");
  if (x86_simd_support.avx512vl)
    printf("-mavx512vl ");
  if (x86_simd_support.erms)
    printf("-erms ");
...
int x86_is_runable(int version) {
  ...
  case SIMD_LVL_BASE:
    return 1;
  case SIMD_LVL_SSE1:
    return x86_simd_support.sse;
  case SIMD_LVL_SSE2:
    return x86_simd_support.sse2;
  case SIMD_LVL_SSE4_1:
    return x86_simd_support.sse41;
  case SIMD_LVL_SSE4_2:
    return x86_simd_support.sse42;
  case SIMD_LVL_AVX2:
    return x86_simd_support.avx2;
  case SIMD_LVL_AVX512F:
    return x86_simd_support.avx512f;
	case SIMD_LVL_ERMS:
    return x86_simd_support.erms;
  }
